...
import { getMyAssignerJobs, modifiedTime, getImageData, getCurrentUser, modifiedTimeByAssignedId } from "../../../Util/NetworkUtils";
...
const AssignedBy = ({ navigation }) => {
    ...
    async function getModifiedTime() {
        try {
            setNewCount(0);
            const newDatas = await modifiedTimeByAssignedId(values);
    ...
    async function getAllMyAssignedJobs() {
        try {
            const newJobs = await getMyAssignerJobs(params);
            if (newJobs.length > 0) {
